...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire('Login Gagal', err.response?.data?.message || 'Login gagal. Mohon periksa kredensial Anda.', 'error');
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-6">
            ...
            <input
              type="password"
              id="password"
              className="shadow-sm appearance-none border border-gray-300 rounded-lg w-full py-2.5 px-3 text-gray-800 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition duration-200 mb-3"
              value={password}
          ...
          <div className="flex items-center justify-between">
            <button
              type="submit"
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-lg focus:outline-none focus:shadow-outline w-full shadow-lg transform hover:scale-[1.01] transition duration-200"
            >
              Sign In
            </button>
          </div>
